    ...
    render() {
        ...
        if (!this.props.inline) {
            ...
            if (this.props.showIcon) {
                ...
                var buttonElement = <Button type="button" icon={this.props.icon}
                                            onClick={(e) => this.onButtonClick(e, this.inputfield)}
        ...
        if (!this.props.timeOnly) {
            /* datepicker header */
            var prevLink = (
                    <a className="ui-datepicker-prev ui-corner-all" href="#" onClick={this.prevMonth.bind(this)}>
                        <span className="fa fa-angle-left"></span>
                    </a>),
                nextLink = (
                    <a className="ui-datepicker-next ui-corner-all" href="#" onClick={this.nextMonth.bind(this)}>
                        <span className="fa fa-angle-right"></span>
                    </a>);

            var currentMonthText = !this.props.monthNavigator && (
                        <span className="ui-datepicker-month">{this.state.currentMonthText} </span>),
                monthNav = this.props.monthNavigator && (
                        <select className="ui-datepicker-month" value={this.state.currentMonth}
                                onChange={(e) => this.onMonthDropdownChange(e.target.value)}>
                            {this.props.locale.monthNames.map((month, i) => <option key={"month_" + i}
                                                                                    value={i}>{month}</option>)}
                        </select>),
                yearNav = this.props.yearNavigator && (
                        <select className="ui-datepicker-year" value={this.state.currentYear}
                                onChange={(e) => this.onYearDropdownChange(e.target.value)}>
                ...
                {this.state.dates.map(function (week, rowIndex) {
                    var columns = week.map(function (date, columnIndex) {
                        ...
                        var dayStyleClass = classNames('ui-state-default', {
                            ...
                            dayLinks = (date.otherMonth ? $this.props.showOtherMonths : true) && (
                                    <a href="#" className={dayStyleClass}
                                       onClick={(e) => $this.onDateSelect(e, date)}>{date.day}</a>);
        ...
        if (this.props.showTime || this.props.timeOnly) {
            ...
            var hourPicker = (<div className="ui-hour-picker">
                    ...
                    <span
                        style={{'display': this.currentHour < 10 ? 'inline' : 'none'}}>0</span><span>{this.currentHour}</span>
                ...
                minutePicker = (<div className="ui-minute-picker">
                    ...
                    <span
                        style={{'display': this.currentMinute < 10 ? 'inline' : 'none'}}>0</span><span>{this.currentMinute}</span>
                ...
                secondPicker = this.props.showSeconds && (<div className="ui-second-picker">
                        ...
                        <span
                            style={{'display': this.currentSecond < 10 ? 'inline' : 'none'}}>0</span><span>{this.currentSecond}</span>
                        <a href="#" onClick={this.incrementSecond.bind(this)}>
                            <span className="fa fa-angle-down"></span>
                        </a>
                    </div>),
        ...
        return (
            <span className={containerStyleClass} style={this.props.style}>
                ...
                <div ref={(el) => this.overlay = ReactDOM.findDOMNode(el)} className={datepickerStyleClass}
                     style={datepickerStyle} onClick={this.onDatePickerClick.bind(this)}>
                    {datepickerHeader}
                    {table}
                    {timepickerHeader}
                </div>
